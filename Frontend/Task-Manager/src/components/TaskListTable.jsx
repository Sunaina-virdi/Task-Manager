...
const TaskListTable = ({tableData}) => {
  ...
  return (
    <div className="overflow-x-auto p-0 rounded-lg mt-3 font-sans">
        <table className='min-w-full'>
            <thead>
                <tr className='text-left'>
                    <th className='py-3 px-4 text-gray-800 font-semibold text-[16px]'>Name</th>
                    <th className='py-3 px-4 text-gray-800 font-semibold text-[16px]'>Status</th>
                    <th className='py-3 px-4 text-gray-800 font-semibold text-[16px]'>Priority</th>
                    <th className='py-3 px-4 text-gray-800 font-semibold text-[16px] hidden md:table-cell'>Created On</th>
                </tr>
            </thead>
            <tbody>
                {tableData.map((task) => (
                    <tr key={task._id} className='border-t border-gray-200 '>
                        <td className='my-4 mx-4 text-gray-700 text-[15px] line-clamp-1 overflow-hidden'>{task.title}</td>
